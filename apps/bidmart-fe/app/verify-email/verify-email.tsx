"use client";

import { useEffect, useMemo, useRef, useState } from "react";
import Link from "next/link";
import { useRouter, useSearchParams } from "next/navigation";
import {
  resendRegistrationVerificationOtp,
  verifyRegistrationEmail,
} from "@/lib/auth/auth-api";

function getErrorMessage(error: unknown) {
  if (error instanceof Error && error.message) {
    return error.message;
  }
  return "Verification request failed. Please try again.";
}

export default function VerifyEmail() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const autoVerifyTriggered = useRef(false);

  const initialEmail = useMemo(
    () => searchParams.get("email")?.trim() || "",
    [searchParams],
  );
  const initialCode = useMemo(
    () => searchParams.get("code")?.trim() || searchParams.get("otp")?.trim() || "",
    [searchParams],
  );
  const initialVerificationToken = useMemo(
    () => searchParams.get("verificationToken")?.trim() || "",
    [searchParams],
  );

  const [email, setEmail] = useState(initialEmail);
  const [code, setCode] = useState(initialCode);
  const [verificationToken, setVerificationToken] = useState(initialVerificationToken);
  const [error, setError] = useState("");
  const [success, setSuccess] = useState("");
  const [isVerifying, setIsVerifying] = useState(false);
  const [isResending, setIsResending] = useState(false);

  useEffect(() => {
    setEmail(initialEmail);
    setCode(initialCode);
    setVerificationToken(initialVerificationToken);
  }, [initialEmail, initialCode, initialVerificationToken]);

  useEffect(() => {
    if (autoVerifyTriggered.current) {
      return;
    }

    if (!initialEmail || !initialCode) {
      return;
    }

    autoVerifyTriggered.current = true;

    const autoVerify = async () => {
      setError("");
      setSuccess("");
      setIsVerifying(true);

      try {
        await verifyRegistrationEmail({
          email: initialEmail,
          code: initialCode,
        });
        setSuccess("Email verified successfully. Redirecting to sign in...");
        setTimeout(() => {
          router.push("/login?registered=true");
        }, 900);
      } catch (autoVerifyError) {
        setError(getErrorMessage(autoVerifyError));
      } finally {
        setIsVerifying(false);
      }
    };

    void autoVerify();
  }, [initialCode, initialEmail, router]);

  const handleSubmit = async (event: React.FormEvent) => {
    event.preventDefault();

    setError("");
    setSuccess("");
    setIsVerifying(true);

    try {
      await verifyRegistrationEmail({ email, code });
      setSuccess("Email verified successfully. Redirecting to sign in...");
      setTimeout(() => {
        router.push("/login?registered=true");
      }, 900);
    } catch (submitError) {
      setError(getErrorMessage(submitError));
    } finally {
      setIsVerifying(false);
    }
  };

  const handleResend = async () => {
    if (!email || !verificationToken) {
      setError("Missing verification session token. Please register again.");
      return;
    }

    setError("");
    setSuccess("");
    setIsResending(true);

    try {
      await resendRegistrationVerificationOtp({ email, verificationToken });
      setSuccess("A new verification code has been sent to your email.");
    } catch (resendError) {
      setError(getErrorMessage(resendError));
    } finally {
      setIsResending(false);
    }
  };

  return (
    <div className="min-h-screen bg-gray-100 px-4 py-12">
      <div className="max-w-xl mx-auto border-3 border-black bg-white shadow-[8px_8px_0_#0A0A0A] p-6 md:p-8">
        <Link href="/" className="inline-flex items-center gap-2 mb-8">
          <div className="w-9 h-9 bg-acid flex items-center justify-center border-2 border-black shadow-[3px_3px_0_#0A0A0A]">
            <span className="text-black font-black text-base">B</span>
          </div>
          <span className="text-xl font-black tracking-tight">BIDMART</span>
        </Link>

        <h1 className="text-3xl md:text-4xl font-black uppercase tracking-tighter mb-2">
          Verify Email
        </h1>
        <p className="text-gray-600 text-sm md:text-base mb-6">
          Confirm your account with the 6-digit OTP from your email.
        </p>

        {error && (
          <div className="mb-4 p-3 bg-hot/10 border-2 border-hot text-hot text-sm font-bold">
            {error}
          </div>
        )}

        {success && (
          <div className="mb-4 p-3 bg-acid/20 border-2 border-black text-black text-sm font-bold">
            {success}
          </div>
        )}

        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label htmlFor="verification-email" className="block text-[10px] md:text-xs font-black text-gray-500 uppercase tracking-widest mb-2">
              Email Address
            </label>
            <input
              id="verification-email"
              type="email"
              value={email}
              onChange={(e) => setEmail(e.target.value)}
              className="input"
              placeholder="you@example.com"
              required
            />
          </div>

          <div>
            <label htmlFor="verification-code" className="block text-[10px] md:text-xs font-black text-gray-500 uppercase tracking-widest mb-2">
              Verification Code
            </label>
            <input
              id="verification-code"
              type="text"
              value={code}
              onChange={(e) => setCode(e.target.value.replace(/\D/g, "").slice(0, 6))}
              className="input text-center text-xl tracking-[0.3em] font-black"
              placeholder="123456"
              maxLength={6}
              required
            />
          </div>

          <button
            type="submit"
            disabled={isVerifying}
            className="btn btn-black w-full text-sm md:text-base font-bold uppercase tracking-wide"
          >
            {isVerifying ? "Verifying..." : "Verify Email"}
          </button>
        </form>

        <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-2">
          <button
            type="button"
            onClick={handleResend}
            disabled={isResending || !verificationToken}
            className="btn text-xs md:text-sm font-bold uppercase"
          >
            {isResending ? "Resending..." : "Resend Code"}
          </button>

          <Link href="/login" className="btn btn-ghost text-xs md:text-sm font-bold uppercase justify-center">
            Back to Login
          </Link>
        </div>

        {!verificationToken && (
          <p className="text-xs text-gray-500 mt-4">
            Resend requires a verification session token from registration flow.
          </p>
        )}
      </div>
    </div>
  );
}
